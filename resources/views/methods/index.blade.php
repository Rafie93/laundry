                                    ...
                                    <div class="form-group col-sm-12">
                                        <div class="row">
                                            <div class="form-group col-lg-8" style="float: left">
                                                <a href="{{Route('method.create')}}" class="btn btn-lg btn-info"> <i class="ace-icon fa fa-plus bigger-110"></i>
                                                    Add New Data</a>
                                            </div>
                    ...
                    <table class="table table-bordered">
                        ...
                        @foreach ($methods as $key=> $row)
                            <tr>
                                ...
                                <td><img src="{{$row->images()}}" alt="" srcset=""></td>
                                <td>{{$row->type}}</td>
                                <td>{{$row->name}}</td>
                                <td>{{$row->outlet_id == null ? 'Semua Outlet' : $row->outlet->name}}</td>
                                <td>
                                    @if ($row->type!="Tunai")
                                    <a href="{{Route('method.edit',$row->id)}}"  class="btn btn-warning">
                     ...
                     {{$methods->appends(request()->except('page'))->links()}}
...
<script>
        $().ready( function () {
            $(".delete").click(function() {
                ...
                function(){
                    setTimeout(function(){
                         window.location =  "/method/"+id+"/delete";
                    }, 2000);
                });
            });
        } );
